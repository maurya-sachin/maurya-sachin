          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              >
                ...
                <span>Resume</span>
                <motion.div className="absolute inset-0 bg-gradient-to-r from-purple-600 to-cyan-500 rounded-xl opacity-0 group-hover:opacity-20 transition-opacity" />
              </motion.button>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <>
            ...
            {/* Mobile Menu Panel */}
            <motion.div
              className={`fixed top-0 right-0 h-full w-80 max-w-[80vw] z-40 lg:hidden ${
                isDarkMode
                ...
                <div className="space-y-6">
                  {navigationItems.map((item, index) => (
                    <motion.button
                      key={item.id}
                      onClick={() => scrollToSection(item.id)}
                      className={`block w-full text-left text-xl font-semibold transition-all duration-300 ${
                        activeSection === item.id
